...
function getApparentLongitude(T) {
    ...
    var phi = earth_1.getNutationInLongitude(T);
    return l + phi;
}
...
function _getSumR(T) {
    ...
    moon_1.MOON_ARGUMENTS_LR.forEach(function (args) {
        ...
        var argF = args[3];
        var argSumR = args[5];
        var tmpSumR = Math.cos(angle_1.deg2rad(argD * D + argMsun * Msun + argMmoon * Mmoon + argF * F));
        switch (argMsun) {
            case 1:
            case -1:
...
function _getSumL(T) {
    ...
    var E = 1 - 0.002516 * T - 0.0000074 * Math.pow(T, 2);
    var sumL = 3958 * Math.sin(angle_1.deg2rad(A1))
        + 1962 * Math.sin(angle_1.deg2rad(L - F))
        + 318 * Math.sin(angle_1.deg2rad(A2));
    moon_1.MOON_ARGUMENTS_LR.forEach(function (args) {
        var argD = args[0];
        var argMsun = args[1];
        var argMmoon = args[2];
        var argF = args[3];
        var argSumL = args[4];
        var tmpSumL = Math.sin(angle_1.deg2rad(argD * D + argMsun * Msun + argMmoon * Mmoon + argF * F));
        switch (argMsun) {
            case 1:
            case -1:
...
function _getSumB(T) {
    ...
    var E = 1 - 0.002516 * T - 0.0000074 * Math.pow(T, 2);
    var sumB = -2235 * Math.sin(angle_1.deg2rad(L))
        + 382 * Math.sin(angle_1.deg2rad(A3))
        + 175 * Math.sin(angle_1.deg2rad(A1 - F))
        + 175 * Math.sin(angle_1.deg2rad(A1 + F))
        + 127 * Math.sin(angle_1.deg2rad(L - Mmoon))
        - 115 * Math.sin(angle_1.deg2rad(L + Mmoon));
    moon_1.MOON_ARGUMENTS_B.forEach(function (args) {
        ...
        var argF = args[3];
        var argSumB = args[4];
        var tmpSumB = Math.sin(angle_1.deg2rad(argD * D + argMsun * Msun + argMmoon * Mmoon + argF * F));
        switch (argMsun) {
            case 1:
            case -1:
