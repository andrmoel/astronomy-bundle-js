    ...
    define('sgp4',[
        ...
        return function (satrec, tsince) {
            ...
            var am, axnl, aynl, betal,
                ...
                sin2u, coseo1, sineo1, cosInc, sinInc, cosip, sinip,
                cosisq, cosAop, sinAop, cosu, sinu,
                delm, delomg, dndt,
                emsq, ecose, el2, eo1, esine,
                argpm, argpp, pl,
                r, v, rdotl, rl, rvdot, rvdotl, argumentOfPerigeeRad,
                t2, t3, t4, tc,
                tem5, temp, temp1, temp2, tempa, tempe, templ,
                u, ux, uy, uz, vx, vy, vz,
                inclm, mm, nm, nodem,
                inclinationRad, xincp, xl, xlm, mp, xmdf, xmx, xmy,
                nodedf, xnode, nodep;
            ...
            if (pl < 0.0) {
                ...
                argumentOfPerigeeRad = Math.atan2(sinu, cosu);
                ...
                argumentOfPerigeeRad = argumentOfPerigeeRad - 0.25 * temp2 * satrec.x7thm1 * sin2u;
                xnode = nodep + 1.5 * temp2 * cosip * sin2u;
                inclinationRad = xincp + 1.5 * temp2 * cosip * sinip * cos2u;
                var mvt = rdotl - nm * temp1 * satrec.x1mth2 * sin2u / constants.xke;
                rvdot = rvdotl + nm * temp1 * (satrec.x1mth2 * cos2u +
                    1.5 * satrec.con41) / constants.xke;

                //  --------------------- orientation vectors -------------------
                sinAop = Math.sin(argumentOfPerigeeRad);
                cosAop = Math.cos(argumentOfPerigeeRad);
                snod = Math.sin(xnode);
                cnod = Math.cos(xnode);
                sinInc = Math.sin(inclinationRad);
                cosInc = Math.cos(inclinationRad);
                xmx = -snod * cosInc;
                xmy = cnod * cosInc;
                ux = xmx * sinAop + cnod * cosAop;
                uy = xmy * sinAop + snod * cosAop;
                uz = sinInc * sinAop;
                vx = xmx * cosAop - cnod * sinAop;
                vy = xmy * cosAop - snod * sinAop;
                vz = sinInc * cosAop;
    ...
    define('dscom',[
        ...
        return function(dscomParameters) {
            ...
            var epoch   = dscomParameters.epoch,
                eccentricity      = dscomParameters.ep,
                aop   = dscomParameters.argpp,
            ...
            var sinomm = Math.sin(aop);
            var cosomm = Math.cos(aop);
