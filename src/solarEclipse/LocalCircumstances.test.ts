...
it('tests getDurationOfPartialEclipse', () => {
    const localCircumstances = new LocalCircumstances(besselianElements, location);

    const result = localCircumstances.getDurationOfPartialEclipse();

    expect(round(result, 1)).toBe(10239.1);
});

it('tests getDurationOfCentralEclipse', () => {
    const localCircumstances = new LocalCircumstances(besselianElements, location);

    const result = localCircumstances.getDurationOfCentralEclipse();
